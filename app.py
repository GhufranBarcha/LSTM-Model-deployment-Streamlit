...
model_soc = tf.keras.models.load_model('lstm_soc_model.h5')
model_soh = tf.keras.models.load_model('lstm_soh_model.h5')
...
st.title("Prediction of SOC & SOH using LSTM")
st.write(
...
This project focuses on building predictive models using Long Short-Term Memory (LSTM) neural networks to forecast both the State of Charge (SOC) and State of Health (SOH) in a battery system. Accurate SOC and SOH predictions are essential for battery management and energy optimization.
...
In time series forecasting, data is typically divided into sequences. `time_steps` specifies the number of previous time steps to consider when predicting the next value. `n_features` is the number of features in the dataset, setting the stage for creating input sequences for the LSTM models.

#### Prepare Dataset

To train LSTM models, the data must be transformed into sequences of inputs (X) and corresponding targets (y). The code loops through the dataset to create overlapping sequences of time steps data points, which will be used to predict the next value (target).
...
Long Short-Term Memory (LSTM) is used as the neural network architecture for sequence modeling. Two LSTM models are created: one for SOC prediction and one for SOH prediction. The models consist of stacked LSTM layers and a final Dense layer for regression. They are compiled with 'mean_squared_error' loss and 'Adam' optimizer.
